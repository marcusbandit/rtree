// Icon tables adapted from eza (MIT/EUPL-1.2 license)
...
/// Return the Nerd Font icon character for a file.
/// `name` is the filename (without path).
/// `is_dir` — whether the entry is a directory.
/// `ext` — lowercase file extension (without the dot), if any.
pub fn icon_for_file(name: &str, is_dir: bool, ext: Option<&str>) -> char {
    if is_dir {
        *DIRECTORY_ICONS.get(name).unwrap_or(&Icons::FOLDER)
    } else if let Some(icon) = FILENAME_ICONS.get(name) {
        *icon
    } else if let Some(e) = ext {
        *EXTENSION_ICONS.get(e).unwrap_or(&Icons::FILE)
    } else {
        Icons::FILE_UNKNOW
    }
}
